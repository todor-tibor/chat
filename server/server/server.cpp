...
int main()
{
	...
	while (1)
	{
		...
		if (temp_id != -1)
		{
			...
			char nev[DEFAULT_BUFLEN] = "";
			recv(client[temp_id].socket, nev, DEFAULT_BUFLEN, 0);
			std::cout << "nev megvan: " << nev;
			client[temp_id].name = nev;

			std::string kliensLista = "1";
			for (size_t i = 0; i < client.size(); i++)
			{
				kliensLista += client.at(i).name + ";";
			}
			send(client[temp_id].socket, kliensLista.c_str(), strlen(kliensLista.c_str()), 0);
